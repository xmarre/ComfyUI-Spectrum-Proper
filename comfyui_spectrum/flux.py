...
def _install_generic_flux_wrapper(inner: Any) -> None:
    ...
    def spectrum_forward_orig(
        ...
        step_ctx = _extract_step_context(options) if isinstance(options, dict) else None
        if runtime is None or not getattr(runtime.cfg, "enabled", False) or step_ctx is None:
...
def _run_flux_forward_with_spectrum(
    ...
    *,
    ...
    feature_dtype = img.dtype
    vec = inner.time_in(timestep_embedding(timesteps, 256).to(feature_dtype))

    if inner.params.guidance_embed and guidance is not None:
        vec = vec + inner.guidance_in(timestep_embedding(guidance, 256).to(feature_dtype))

    if inner.vector_in is not None:
        if y is None:
            y = torch.zeros((img.shape[0], inner.params.vec_in_dim), device=img.device, dtype=feature_dtype)
